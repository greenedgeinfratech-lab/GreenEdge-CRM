...
export default function DesignationsSettingsPage() {
  ...
  const fetchDesignations = async (q = '') => {
    try {
      setLoading(true);
      const res = await api.get(`/designations/?search=${q}`);
      const resData = res.data;
      const rawList = resData?.data?.results || resData?.data || resData?.results || resData;
      setDesignations(Array.isArray(rawList) ? rawList : []);
    } catch (err) {
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">Manage job designations/titles.</p>
        </div>
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {loading ? (
              ...
              <tr><td colSpan={4} className="px-6 py-4 text-center text-sm text-gray-500">No designations found.</td></tr>
            ) : (
              designations.map((d) => (
                <tr key={d.id}>
