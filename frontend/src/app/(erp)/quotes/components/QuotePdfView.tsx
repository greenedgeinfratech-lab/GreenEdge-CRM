...
export default function QuotePdfView({ quote }: QuotePdfViewProps) {
  ...
  const title = quote.type === 'Proforma Invoice' ? 'PROFORMA INVOICE' : 'QUOTATION';
    ...
    <div className="bg-white text-black p-6 font-sans text-xs max-w-[1000px] mx-auto border border-gray-300 print:border-none print:p-0">
      ...
      <div className="flex justify-between text-[11px] font-medium border-t border-b border-gray-300 py-1.5 px-1 mb-2">
        ...
        <div className="flex gap-6">
          ...
          <div>
            <span>Date: </span>
            <span className="font-bold">{quote.quote_date || new Date().toLocaleDateString('en-GB')}</span>
          </div>
          <div>
            <span>Valid Until: </span>
            <span className="font-bold">{quote.valid_until || '-'}</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 border border-black mb-3">
        <div className="p-2 border-r border-black">
          ...
          <div className="font-semibold text-gray-900">{quote.lead_name || quote.customer_name || 'N/A'}</div>
          <div className="whitespace-pre-line text-gray-700">{quote.address || quote.billing_address || 'N/A'}</div>
        </div>
        <div className="p-2">
          <div className="font-bold underline mb-1 text-[11px]">Shipping Address</div>
          <div className="font-semibold text-gray-900">{quote.lead_name || quote.customer_name || 'N/A'}</div>
          <div className="whitespace-pre-line text-gray-700">{quote.shipping_address || quote.address || 'N/A'}</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-12 border border-black mb-3">
        <div className="col-span-5 border-r border-black p-2 text-[10px]">
          ...
          <div className="whitespace-pre-line text-gray-800 leading-relaxed">
            {quote.bank_details || 'State Bank of India\nA/C No: 43279551326\nIFSC: SBIN0001234\nBranch: Aligarh Main'}
          </div>
        </div>
        ...
        <div className="col-span-4 text-[10px]">
          <div className="flex justify-between border-b border-black p-1">
            ...
            <span className="font-semibold">₹{(Number(quote.total_taxable) || 0).toFixed(2)}</span>
          </div>
          {isIgst ? (
            <div className="flex justify-between border-b border-black p-1">
              <span>Add IGST</span>
              <span className="font-semibold">₹{(Number(quote.total_igst) || 0).toFixed(2)}</span>
            </div>
          ) : (
            <>
              <div className="flex justify-between border-b border-black p-1">
                <span>Add CGST</span>
                <span className="font-semibold">₹{(Number(quote.total_cgst) || 0).toFixed(2)}</span>
              </div>
              <div className="flex justify-between border-b border-black p-1">
                <span>Add SGST</span>
                <span className="font-semibold">₹{(Number(quote.total_sgst) || 0).toFixed(2)}</span>
              </div>
            </>
          )}
      ...
      {quote.terms_conditions && quote.terms_conditions.length > 0 && (
        <div className="mb-3 text-[10px]">
          ...
          <ol className="list-decimal list-inside space-y-0.5 text-gray-700">
            {quote.terms_conditions.map((t: any, i: number) => (
      ...
      <div className="flex justify-between items-end border-b border-black pb-4 mb-3">
        <div className="text-[9px] text-gray-600 flex flex-col gap-1">
          <p>This is a computer generated {quote.type || 'Quotation'}.</p>
          <p>E. & O. E.</p>
        </div>
