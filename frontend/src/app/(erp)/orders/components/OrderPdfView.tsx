...
export default function OrderPdfView({ order }: OrderPdfViewProps) {
  ...
  return (
    <div className="bg-white text-black p-6 font-sans text-xs max-w-[1000px] mx-auto border border-gray-300 print:border-none print:p-0">
      ...
      <div className="flex justify-between items-start border-b border-black pb-3">
        <div>
          ...
          <div className="flex items-center gap-4 text-[10px] text-gray-700 mt-1">
            <span>Phone: 9837067681</span>
            <span>Email: greenedgeinfratech@gmail.com</span>
          </div>
        </div>
      ...
      <div className="text-center my-3">
        <h2 className="text-sm font-bold uppercase tracking-wider text-black">PURCHASE ORDER</h2>
      </div>
      ...
      <div className="flex justify-between text-[11px] font-medium border-t border-b border-gray-300 py-1.5 px-1 mb-2">
        ...
        <div className="flex gap-6">
          <div>
            <span>Order No.: </span>
            <span className="font-bold">{order.order_number || 'Draft'}</span>
          </div>
          <div>
            <span>Date: </span>
            <span className="font-bold">{order.order_date || new Date().toLocaleDateString('en-GB')}</span>
          </div>
          {order.due_date && (
            <div>
              <span>Due Date: </span>
              <span className="font-bold">{order.due_date}</span>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 border border-black mb-3">
        <div className="p-2 border-r border-black">
          <div className="font-bold underline mb-1 text-[11px]">Supplier / Billing Address</div>
          <div className="font-semibold text-gray-900">{order.customer_name || 'N/A'}</div>
          <div className="whitespace-pre-line text-gray-700">{order.address || 'N/A'}</div>
        </div>
        <div className="p-2">
          <div className="font-bold underline mb-1 text-[11px]">Shipping Address</div>
          <div className="font-semibold text-gray-900">{order.customer_name || 'N/A'}</div>
          <div className="whitespace-pre-line text-gray-700">{order.shipping_address || order.address || 'N/A'}</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-12 border border-black mb-3">
        <div className="col-span-5 border-r border-black p-2 text-[10px]">
          ...
          <div className="whitespace-pre-line text-gray-800 leading-relaxed">
            {order.bank_details || 'State Bank of India\nA/C No: 43279551326\nIFSC: SBIN0001234\nBranch: Aligarh Main'}
          </div>
        </div>
        <div className="col-span-3 border-r border-black p-2 text-[10px]">
          <div className="font-bold underline mb-1">Total Amount in Words:</div>
        ...
        <div className="col-span-4 text-[10px]">
          <div className="flex justify-between border-b border-black p-1">
            ...
            <span className="font-semibold">₹{(Number(order.total_taxable) || 0).toFixed(2)}</span>
          </div>
          {isIgst ? (
            <div className="flex justify-between border-b border-black p-1">
              <span>Add IGST</span>
              <span className="font-semibold">₹{(Number(order.total_igst) || 0).toFixed(2)}</span>
            </div>
          ) : (
            <>
              <div className="flex justify-between border-b border-black p-1">
                <span>Add CGST</span>
                <span className="font-semibold">₹{(Number(order.total_cgst) || 0).toFixed(2)}</span>
              </div>
              <div className="flex justify-between border-b border-black p-1">
                <span>Add SGST</span>
                <span className="font-semibold">₹{(Number(order.total_sgst) || 0).toFixed(2)}</span>
              </div>
            </>
          )}
