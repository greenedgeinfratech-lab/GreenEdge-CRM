...
class Invoice(TenantBaseModel):
    """
    Model representing an Invoice (B2B or Cash Memo).
    """
    type = models.CharField(max_length=50, default='B2B Invoice')
    ...
    customer_ledger = models.CharField(max_length=100, blank=True, null=True)
    income_ledger = models.CharField(max_length=100, blank=True, null=True)
    ...
    recovery_amt = models.DecimalField(max_digits=14, decimal_places=2, default=0)
    recovery_notes = models.TextField(blank=True, null=True)
    
    invoice_status = models.CharField(max_length=50, default='Unpaid')
    ...
    def __str__(self):
        return f"{self.invoice_no or 'Draft'} - {self.customer_name}"
